...
export default class Automatedtourgenerator extends Component {
  ...
  calculateTripDays = async () => {
    ...
    if (this.state.transportType != 'Personal') {
      budgetforHotel =
        this.state.budget - this.state.vehiclePrice * this.state.tripDays;
    } else {
    ...
    for (var i = 0; i < this.state.selectedPois.length; i++) {
      console.log(
        'zee12  ' + i + ' ' + JSON.stringify(this.state.selectedPois[i].photo)
      );
    }

    if (
      this.state.transportType == 'Personal' &&
      this.state.selectedHotel.price *
        this.state.tripDays *
        this.state.inputDetails.Adults >
        this.state.budget
    ) {
      this.setState({
        budgetExceeding: true,
    ...
    if (
      ...
      this.state.selectedHotel.price *
        this.state.tripDays *
        this.state.inputDetails.Adults +
        this.state.vehiclePrice * this.state.tripDays >
        this.state.budget
    ) {
      this.setState({
        budgetExceeding: true,
  ...
  searchHotel = async (startRange, endRange) => {
    ...
    for (var i = 0; i < this.state.allHotels.length; i++) {
      ...
      if (
        this.state.allHotels[i].price > startRange &&
        this.state.allHotels[i].price < endRange
      ) {
        await this.setState({
  ...
  async componentDidMount() {
    ...
    let selectedPoiPosition = this.state.selectedPois.map(
      (postion) => postion.latLng
    );
    this.state.selectedPoiPosition = selectedPoiPosition;
    console.log(
      'selected poiss location' + JSON.stringify(selectedPoiPosition)
    );
    ...
    if (this.state.transportType == 'Bus') {
      ...
      await this.setState({
        tourDay: 2,
        locationTimeInterval: [
          ...this.state.locationTimeInterval,
          this.state.inputDetails.travelTime,
        ],
      });
    } else if (
      this.state.transportType != 'Bus' &&
      this.state.inputDetails.stayCity == null
    ) {
      console.log('zee22');
      await this.setState({
        tourDay: 2,
      ...
      console.log('zee33');
      var locationTimeIntervals = await [
        this.directionRenderer(
          this.state.inputDetails.source,
    ...
    let startPoi =
      selectedPoiPosition[0][0].lat + ',' + selectedPoiPosition[0][0].lng;
    console.log('No I am first');
    let hotelPosition =
      this.state.selectedHotel.latLng[0].lat +
      ',' +
      this.state.selectedHotel.latLng[0].lng;
    console.log(
      'Hotel position ' + hotelPosition + '\nPOi postion ' + startPoi
    );
    await this.hoteldirectionRenderer(hotelPosition, startPoi);
    for (var i = 0, j = 1; i < selectedPoiPosition.length - 1; i++, j++) {
      let startPoint =
        selectedPoiPosition[i][0].lat + ',' + selectedPoiPosition[i][0].lng;
      let endPoint =
        selectedPoiPosition[j][0].lat + ',' + selectedPoiPosition[j][0].lng;
    ...
    for (var i = 0; i <= totalDays; i++) {
      var new_date = moment(this.state.currentDate, 'YYYY:MM:DD').add(
        1,
        'days'
      );
    ...
    await this.setState({
      ...
      hotelPrice:
        price * (this.state.tripDays * (this.state.inputDetails.Adults / 2)),
    });
    ...
    await axios
      ...
      .then((res) => {
        ...
        if (res.status === 200) {
          ...
          var fuelCost = parseInt(
            (res.data.route.fuelUsed * 3.78541).toFixed(2)
          );
          ...
          console.log(
            'Hotel to first POI time is ' + this.state.hotelToFirstPoiTime
          );
  ...
  getTags = async () => {
    await axios
      .get('http://localhost:3000/users/getTags')
      .then((res) => {
        this.setState({
    ...
    await axios
      ...
      .then((res) => {
        ...
        if (res.status === 200) {
          ...
          console.log(
            'Source Destination time is ' + this.state.locationTimeInterval
          );
          console.log('zeebash' + fuelCost);
        } else {
          console.log('Cannot find the time interval.');
  ...
  saveTour = async (e) => {
    ...
    if (
      (this.state.tagHotelTypeValue &&
        ...
        this.state.tagLocationTypeValue &&
        this.state.tagTripTypeValue) == null
    ) {
      toast.warning('Fill all the fields');
    } else {
      let cost = parseInt(
        this.state.vehiclePrice * this.state.tripDays + this.state.hotelPrice
      );
      let confirm;
      if (this.state.budgetExceeding) {
        confirm = window.confirm(
          'Do you want to exceed your trip cost to ' +
            cost.toLocaleString() +
            ' PKR ?'
        );
        if (confirm) {
      ...
      return (
        <div>
          ...
          <Timeline style={{ marginLeft: '-80%' }}>
            ...
            >
              <PlaceIcon style={{ marginTop: '-0.5%', fontSize: '19px' }} />
              {this.state.inputDetails.source}
            </h4>
            <h4
              style={{ textAlign: 'left', marginLeft: '47%', fontSize: '20px' }}
            >
              Day {props.day}{' '}
              <span
                style={{
            ...
            <TimelineItem>
              ...
              <TimelineContent>
                <Typography>{this.state.inputDetails.source}</Typography>
        ...
        <div>
          <Timeline style={{ marginLeft: '-80%' }}>
            ...
            <TimelineItem>
              ...
              <TimelineContent>
                <Typography>{this.state.inputDetails.stayCity}</Typography>
                <Typography variant="body2" color="textSecondary">
                  {this.state.startTime} am
                </Typography>
              </TimelineContent>
      ...
      return (
        <div>
          ...
          >
            ...
            >
              {this.state.tourDay > this.state.allTourDates.length
                ? 'Your selected Dates exceeded '
                : this.state.allTourDates[this.state.tourDay - 1]}
            </span>
          </h4>
          <TimelineItem>
            <TimelineSeparator>
            ...
            <TimelineContent>
              <Typography>{this.state.inputDetails.destination}</Typography>
              <Typography variant="body2" color="textSecondary">
                {this.state.selectedHotel.name}
              </Typography>
            </TimelineContent>
      ...
      <>
        {!this.state.isLoading ? (
          <div id="tourgeneratorBox">
            ...
            <List
              style={{ width: '320px', marginLeft: '5.5%', marginTop: '-1%' }}
            >
              <ListItem>
                <ListItemAvatar>
                  <Avatar>
                ...
                <ListItemText
                  color="secondary"
                  primary="Source"
                  secondary={this.state.inputDetails.source}
                />
              </ListItem>
              {this.state.inputDetails.stayCity == null ? (
                <></>
                ...
                <ListItem>
                  ...
                  <ListItemText
                    primary="Stay City"
                    secondary={this.state.inputDetails.stayCity}
                  />
                </ListItem>
              )}
              <ListItem>
                ...
                <ListItemText
                  primary="Destination"
                  secondary={this.state.inputDetails.destination}
                />
              </ListItem>
              <ListItem>
                ...
                <ListItemText
                  primary="Total Days"
                  secondary={this.state.tripDays + ' Days'}
                />
              </ListItem>
              {this.state.budgetExceeding ? (
                <ListItem>
                  <ListItemAvatar color="secondary">
                    <Avatar>
              ...
              <ListItem>
                ...
                <ListItemText
                  primary="Hotel & Accomodation"
                  secondary={
                    this.state.hotelPrice.toLocaleString() +
                    ` PKR (${this.state.tripDays} Days for ${this.state.inputDetails.Adults} people)`
                  }
                />
              </ListItem>
              <ListItem>
                ...
                <ListItemText
                  primary="Fuel "
                  secondary={this.state.fuelCost + ' Liters Approx'}
                />
              </ListItem>
              {this.state.transportType == 'Rental' ? (
                <ListItem>
                  <ListItemAvatar color="secondary">
                    <Avatar>
                      <DirectionsCarIcon />
                  ...
                  <ListItemText
                    primary={
                      'Vehicle Cost ' +
                      this.state.inputDetails.selectedVehicle.name
                    }
                    secondary={
                      (
                        this.state.vehiclePrice * this.state.totalDays
                      ).toLocaleString() + ` PKR (${this.state.tripDays} Days)`
                    }
                  />
                </ListItem>
              ) : (
              ...
              {this.state.transportType == 'Bus' ? (
                <ListItem>
                  ...
                  <ListItemText
                    primary="Vehicle cost"
                    secondary={
                      (
                        this.state.vehiclePrice * this.state.tripDays
                      ).toLocaleString() +
                      ` PKR (${
                        this.state.tripDays - 1
                      } Days) Including Bus Fare`
                    }
                  />
                </ListItem>
              ) : (
              ...
              <ListItem>
                ...
                <ListItemText
                  primary="User Budget"
                  secondary={this.state.budget.toLocaleString() + ' PKR'}
                />
              </ListItem>
            </List>
            {this.state.inputDetails.stayCity == null ? (
              <FirstDayTripCar day={this.state.tourDay - 1} />
            ) : (
              <FirstDayTripCar day={this.state.tourDay - 2} />
            )}
            {this.state.inputDetails.stayCity == null ? (
              <></>
            ) : (
              <SecondDayTripCar day={this.state.tourDay - 1} />
            )}
            <Timeline style={{ marginLeft: '-80%' }}>
              <h4
                style={{
              ...
              <TimelineItem>
                ...
                <TimelineContent>
                  <Typography>{this.state.selectedHotel.name}</Typography>
                  <Typography variant="body2" color="textSecondary">
                    &nbsp;&nbsp;{this.state.startTime} am
                  </Typography>
                </TimelineContent>
              ...
              {this.state.selectedPois.map((poi, index) => (
                <div>
                  {index == 0 ? (
                    <></>
                  ) : index % 3 == 0 ? (
                    <StartingFromHotel index={index} />
                  ) : (
                    <></>
                  )}
                  <TimelineItem>
                        ...
                        <div className="row no-gutters">
                          <div className="col-md-4">
                            {poi.photo != ' ' ? (
                              <img
                                ...
                                className="card-img"
                                style={{ height: '200px', marginLeft: '-3%' }}
                              />
                            ) : (
                              <img
                          ...
                          <div className="col-md-8">
                            <div className="card-body">
                              ...
                              {poi.rating === undefined || poi.rating < 1 ? (
                                <p
                                  style={{
                                    textAlign: 'right',
                                    marginTop: '-6%',
                  ...
                  <TimelineItem>
                    ...
                    <TimelineContent>
                      ...
                      <Typography variant="body2" color="textSecondary">
                        {index == 2 ||
                        index == this.state.selectedPois.length - 1
                          ? this.state.poiToHotelTime[index] + ' hours'
                          : this.state.poiTimeIntervals[index] + ' hours'}
                      </Typography>
                    </TimelineContent>
                  </TimelineItem>{' '}
                  {index == 2 && index != this.state.selectedPois.length - 1 ? (
                    <EndingTowardsHotel tripEnd={false} />
                  ) : index == this.state.selectedPois.length - 1 ||
                    (index + 1) % 3 == 0 ? (
                    <EndingTowardsHotel tripEnd={true} />
                  ) : (
                    <></>
                  )}
                </div>
              ...
              <Alert
                ...
                message={
                  <TextLoop mask>
                    ...
                    <div>
                      Your Trip duration is for {this.state.tripDays} days.
                    </div>
                ...
                >
                  ...
                  {Array.isArray(this.state.tagWeatherType) && (
                    <select
                      ...
                      onChange={(e) =>
                        this.setState({ tagWeatherTypeValue: e.target.value })
                      }
                    >
                      <option>--</option>
                ...
                >
                  ...
                  {Array.isArray(this.state.tagHotelType) && (
                    <select
                      ...
                      style={{ fontSize: '12px', width: '90px' }}
                      onChange={(e) =>
                        this.setState({ tagHotelTypeValue: e.target.value })
                      }
                    >
                      <option>--</option>
                ...
                >
                  ...
                  {Array.isArray(this.state.tagTripType) && (
                    <select
                      ...
                      onChange={(e) =>
                        this.setState({ tagTripTypeValue: e.target.value })
                      }
                    >
                      <option>--</option>
                ...
                >
                  ...
                  {Array.isArray(this.state.tagLocationType) && (
                    <select
                      ...
                      style={{ fontSize: '12px', width: '100px' }}
                      onChange={(e) =>
                        this.setState({ tagLocationTypeValue: e.target.value })
                      }
                    >
                      <option>--</option>
          ...
          <div style={{ marginLeft: '49%', marginTop: '15%' }}>
            <Spin size="large" />
            <h4 style={{ marginLeft: '-8%', marginTop: '1%' }}>
              Generating Trip
            </h4>
          </div>
        )}{' '}
